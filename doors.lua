...
local B=setmetatable({instance=A,destructed=false},y)B._destroyListener=B.instance.AncestryChanged:Connect(function(C,D)if D==nil then B:destroy()end end) return B:constructor(A)end function y.constructor(A,B)A.root=B:WaitForChild('HumanoidRootPart',5)A.humanoid=B:WaitForChild('Humanoid',5)y.attached[B]=A task.defer(A.onStart,A)return A end function y.onStart(A)A.instance.ChildAdded:Connect(function(B)if B:IsA'Tool'then A:onToolAdded(B)end end)end function y.onToolAdded(A,B)if B.Name=='LibraryHintPaper'then if not Toggles.PADLOCK_CODE.Value then return end 
local C=B:WaitForChild('UI',1)if C then 
local D={}for E=1,5 do 
local F=C:FindFirstChild(tostring(E),1)if F then 
local G=v[F.ImageRectOffset.X/50]if G~=nil then D[E]=G end end end 
local E=''for F=1,5 do
local G=D[F]E=E..(G or'_')end k:Notify('[Code Parser]: '..E,10)end end end function y.onPhysics(A,B,C)
local D=Options.SPRINT_BOOST.Value if D>0 and Toggles.SPRINT_ENABLED.Value and Options.SPRINT_KEYBIND:GetState()then 
local E=A.humanoid.MoveDirection*z if E.Magnitude>0 then A.instance:TranslateBy(E*D*C)end end end function y.destroy(A)if A.destructed then return else A.destructed=true\A._destroyListener:Disconnect()end y.attached[A.instance]=nil end end 
local z do z={}z.attached={}z.__index=z function z.new(A)local B=setmetatable({instance=A,destructed=false},z)B._destroyListener=B.instance.AncestryChanged:Connect(function(C,D)if D==nil then B:destroy()end end)return B:constructor(A)end function z.constructor(A,B)A.root=B:WaitForChild('HumanoidRootPart',5)A.humanoid=B:WaitForChild('Humanoid',5)z.attached[B]=A task.defer(A.onStart,A)return A end function z.onStart(A)end function z.render(A)end function z.destroy(A)if A.destructed then return else A.destructed=true A._destroyListener:Disconnect()end z.attached[A.instance]=nil end end 
local A do A={}A.__index=A function A.new(B)
local C=setmetatable({instance=B,destructed=false},A)C._destroyListener=b.PlayerRemoving:Connect(function(D)if D==B then C:destroy()end end)return C:constructor(B)end function A.constructor(B,C)task.defer(B.onStart,B)return B end function A.onStart(B)
local C=B.instance C.CharacterAdded:Connect(function(D)B:onCharacterAdded(D)end)
local D=C.Character if D then B:onCharacterAdded(D)end end function A.onCharacterAdded(B,C)z.new(C)end function A.destroy(B)if B.destructed then return else B.destructed=true end end end 
local B do B={}B.attached={}B.__index=B local C=o.WorldToViewportPoint 
local D,E=Vector2.new,CFrame.new 
local F=E(0,1,0)function B.new(G)
local H=setmetatable({instance=G,destructed=false},B)H._destroyListener=H.instance.AncestryChanged:Connect(function(I,J)if J==nil then H:destroy()end end)return H:constructor(G)end function B.constructor(G,H)
local I=H 
local J=I:WaitForChild('FigureRagdoll',2.5) G.root=J:WaitForChild('Root',2.5)G.quad=Drawing.new'Quad'task.defer(function()G:onStart()B.attached[H]=G end)return G end function B.onStart(G)
local H=G.quad H.Visible=false H.Transparency=0.75 H.Color=Color3.new(1,0,0)H.Thickness=3 end function B.render(G)
local H=G.quad 
local I=G.root if Toggles.FIGURE_ESP.Value then 
local J=I.CFrame*F 
local K=I.Size*1.75 
local L=C(o,(J*E(K.X,K.Y,0)).Position)
local M=C(o,(J*E(-K.X,K.Y,0)).Position)
local N=C(o,(J*E(-K.X,-K.Y,0)).Position)
local O=C(o,(J*E(K.X,-K.Y,0)).Position)if math.min(L.Z,M.Z,N.Z,O.Z)>0 then H.PointA=D(M.X,M.Y)H.PointB=D(L.X,L.Y)H.PointC=D(O.X,O.Y)H.PointD=D(N.X,N.Y)H.Visible=true else H.Visible=false end else H.Visible=false end end function B.destroy(G)if G.destructed then return else G.destructed=true G._destroyListener:Disconnect()end B.attached[G.instance]=nil G.quad:Remove()end end 
local C do C={}C.attached={}C.__index=C 
local D=o.WorldToViewportPoint 
local E,F=Vector2.new,CFrame.new function C.new(G)
local H=setmetatable({instance=G,destructed=false},C)H._destroyListener=H.instance.AncestryChanged:Connect(function(I,J)if J==nil then H:destroy()end end)return H:constructor(G)end function C.constructor(G,H)G.quad=Drawing.new'Quad'G.root=H.PrimaryPart or H:WaitForChild('RushNew',5)task.defer(function()G:onStart()C.attached[H]=G end)return G end function C.onStart(G)
local H=G.quad H.Visible=false H.Transparency=0.75 H.Color=Color3.new(1,0,0)H.Thickness=2 end 
local G=Vector3.new(0.4,0.8,0)function C.render(H)
local I=H.quad
local J=H.root if Toggles.RUSH_ESP.Value then 
local K=J.CFrame 
local L=J.Size*G
local M=D(o,(K*F(L.X,L.Y,0)).Position)
local N=D(o,(K*F(-L.X,L.Y,0)).Position)
local O=D(o,(K*F(-L.X,-L.Y,0)).Position)
local P=D(o,(K*F(L.X,-L.Y,0)).Position) if math.min(M.Z,N.Z,O.Z,P.Z)>0 then I.PointA=E(N.X,N.Y)I.PointB=E(M.X,M.Y)I.PointC=E(P.X,P.Y)I.PointD=E(O.X,O.Y)I.Visible=true else I.Visible=false end else I.Visible=false end end function C.destroy(H)if H.destructed then return else H.destructed=true H._destroyListener:Disconnect()end C.attached[H.instance]=nil H.quad:Remove()end end 
local D do D={}D.attached={}D.__index=D 
local E=o.WorldToViewportPoint 
local F,G=Vector2.new,CFrame.new function D.new(H,I)
local J=setmetatable({instance=H,destructed=false},D)J._destroyListener=J.instance.AncestryChanged:Connect(function(K,L)if L==nil then J:destroy()end end)return J:constructor(H,I)end function D.constructor(H,I,J)H.roomNumber=J H.quad=Drawing.new'Quad'task.defer(function()H:onStart()D.attached[I]=H end)return H end function D.onStart(H)
local I=H.quad I.Visible=false I.Transparency=0.6 I.Thickness=2 end function D.render(H)
local I=H.quad 
local J=H.instance.Door if s.LatestRoom.Value-H.roomNumber>2 then I.Visible=false return end if Toggles.DOOR_ESP.Value then 
local K=J.CFrame 
local L=J.Size/2 
local M=E(o,(K*G(L.X,L.Y,0)).Position)
local N=E(o,(K*G(-L.X,L.Y,0)).Position)
local O=E(o,(K*G(-L.X,-L.Y,0)).Position)
local P=E(o,(K*G(L.X,-L.Y,0)).Position)if math.min(M.Z,N.Z,O.Z,P.Z)>0 then I.PointA=F(N.X,N.Y)I.PointB=F(M.X,M.Y)I.PointC=F(P.X,P.Y)I.PointD=F(O.X,O.Y)I.Color=Options.DOOR_ESP_COLOR.Value I.Visible=true else I.Visible=false end else I.Visible=false end end function D.destroy(H)if H.destructed then return else H.destructed=true H._destroyListener:Disconnect()end D.attached[H.instance]=nil H.quad:Remove()end end 
local E do E={}E.attached={}E.__index=E 
local F=o.WorldToViewportPoint 
local G,H=Vector2.new,CFrame.new 
local I=H(0,0,-0.5) function E.new(J,K)
local L=setmetatable({instance=J,destructed=false},E)L._destroyListener=L.instance.AncestryChanged:Connect(function(M,N)if N==nil then L:destroy()end end)return L:constructor(J,K)end function E.constructor(J,K,L)J.roomNum=L J.root=K:WaitForChild('Main',2.5)J.occupied=K:WaitForChild('HiddenPlayer',2.5)J.quad=Drawing.new'Quad'task.defer(function()J:onStart()E.attached[K]=J end)return J end function E.onStart(J)if J.destructed then return end 
local K=J.quad K.Visible=false K.Thickness=2 end function E.render(J)
local K=J.quad 
local L=J.root 
local M=J.occupied.Value if not M and Toggles.WARDROBE_ESP.Value then 
local N=L.CFrame*I 
local O=L.Size*0.55 
local P=F(o,(N*H(O.X,O.Y,0)).Position)
local Q=F(o,(N*H(-O.X,O.Y,0)).Position)
local R=F(o,(N*H(-O.X,-O.Y,0)).Position)
local S=F(o,(N*H(O.X,-O.Y,0)).Position)if math.min(P.Z,Q.Z,R.Z,S.Z)>0 then K.PointA=G(Q.X,Q.Y)K.PointB=G(P.X,P.Y)K.PointC=G(S.X,S.Y)K.PointD=G(R.X,R.Y)K.Color=Options.WARDROBE_ESP_COLOR.Value K.Transparency=(s.LatestRoom.Value-J.roomNum>1)and 0.1 or 0.4 K.Visible=true else K.Visible=false end else K.Visible=false end end function E.destroy(J)if J.destructed then return else J.destructed=true J._destroyListener:Disconnect()end E.attached[J.instance]=nil J.quad:Remove()end end 
local F do F={}F.attached={}F.__index=F 
local G=o.WorldToViewportPoint 
local H=Vector2.new(0,3)function F.new(I,J)
local K=setmetatable({instance=I,destructed=false},F)K._destroyListener=K.instance.AncestryChanged:Connect(function(L,M)if M==nil then K:destroy()end end)return K:constructor(I,J)end function F.constructor(I,J,K)I.name=K or J.Name I.label=Drawing.new'Text'I.indicator=Drawing.new'Circle'I.outline=Drawing.new'Circle'task.defer(function()I:onStart()F.attached[J]=I end)return I end function F.onStart(I)
local J,K,L=I.label,I.indicator,I.outline J.Text=I.name J.Size=18 J.Center=true J.Outline=true J.Visible=false K.Thickness=1 K.NumSides=12 K.Radius=5 K.Filled=true K.Visible=false L.Color=Color3.new()L.Thickness=2 L.NumSides=12 L.Radius=5 L.Filled=false L.Visible=false L.ZIndex=2 end function F.render(I)
local J,K,L=I.label,I.indicator,I.outline if Toggles.OBTAINABLE_ESP.Value then
local M,N=G(o,I.instance.Position)
local O=N and M.Z>2.5 if O then 
local P=Vector2.new(M.X,M.Y)J.Position=P+H K.Position=P L.Position=P 
local Q=Options.OBTAINABLE_ESP_COLOR.Value J.Color=Q K.Color=Q end J.Visible=O K.Visible=O L.Visible=O else J.Visible=false K.Visible=false L.Visible=false end end functionF.destroy(I)if I.destructed then return else I.destructed=true I._destroyListener:Disconnect()end F.attached[I.instance]=nil I.label:Remove()I.indicator:Remove()I.outline:Remove()end end 
local G do G={}G.attached={}G.__index=G 
local H=o.WorldToViewportPoint 
local I=Vector2.new(0,3)function G.new(J,K)
local L=setmetatable({instance=J,destructed=false},G)L._destroyListener=L.instance.AncestryChanged:Connect(function(M,N)if N==nil then L:destroy()end end) return L:constructor(J,K)end function G.constructor(J,K,L)J.name=L or K.Name J.label=Drawing.new'Text'J.indicator=Drawing.new'Circle'J.outline=Drawing.new'Circle'task.defer(function()J:onStart()G.attached[K]=J end)return J end function G.onStart(J)
local K,L,M=J.label,J.indicator,J.outline K.Text=J.name K.Size=18 K.Center=true K.Outline=true K.Visible=false L.Thickness=1 L.NumSides=12 L.Radius=5 L.Filled=true L.Visible=false M.Color=Color3.new()M.Thickness=2 M.NumSides=12 M.Radius=5 M.Filled=false M.Visible=false M.ZIndex=2 end function G.render(J)
local K,L,M=J.label,J.indicator,J.outline if Toggles.INTERACTABLE_ESP.Value then 
...
local L=J:WaitForChild('Assets',5)for M,N in ipairs(L:GetChildren())do task.defer(K,N)end L.ChildAdded:Connect(K)end function H.destroy(I)if I.destructed then return else I.destructed=true I._destroyListener:Disconnect()end end end k:SetWatermark'Linoria Community (OminousVibes)'k:Notify'Loading UI...'do 
local I=k:CreateWindow'Doors'do 
